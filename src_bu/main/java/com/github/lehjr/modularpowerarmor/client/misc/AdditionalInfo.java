        ...
        if (doAdditionalInfo()) {
            stack.getCapability(PowerModuleCapability.POWER_MODULE).ifPresent(pm->{
                if (pm.getCategory() == EnumModuleCategory.ARMOR) {
                    double pysArmor = pm.applyPropertyModifiers(Constants.ARMOR_VALUE_PHYSICAL);
                    double energyArmor = pm.applyPropertyModifiers(Constants.ARMOR_VALUE_ENERGY);
//                    double toughness = pm.applyPropertyModifiers(MPSConstants)
                    double knockbackResistance = pm.applyPropertyModifiers(Constants.KNOCKBACK_RESISTANCE);
...
//
//                // Water tank info
//                FluidUtils fluidUtils = new FluidUtils(player, stack, MPSModuleConstants.MODULE_BASIC_COOLING_SYSTEM__REGNAME);
...
//                fluidUtils = new FluidUtils(player, stack, MPSModuleConstants.MODULE_ADVANCED_COOLING_SYSTEM__REGNAME);
