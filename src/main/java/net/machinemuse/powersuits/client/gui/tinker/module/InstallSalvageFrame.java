...
import net.machinemuse.powersuits.client.gui.common.InventoryFrame;
import net.machinemuse.powersuits.client.gui.common.ItemSelectionFrame;
import net.machinemuse.powersuits.client.sound.SoundDictionary;
import net.machinemuse.powersuits.containers.IHideableSlot;
...
//                new Colour(0.545D, 0.545D, 0.545D, 1),
        ...
        this.craftAndInstallButton = new ClickableButton(new TranslationTextComponent("gui.powersuits.craftAndInstall"),
    ...
    public void init(double left, double top, double right, double bottom) {
        ...
        MusePoint2D arrowsCenter =
                // center of crafting grid
                new MusePoint2D(border.right() -14 - craftingGridSize/2,
                        // center between grid and border bottom
                        (border.finalBottom() - ( border.finalHeight() - craftingGridSize - 7)/2));

        this.forwardArrow.setTargetDimensions(arrowsCenter.plus(10, -10), new MusePoint2D(12, 17));
        this.backArrow.setTargetDimensions(arrowsCenter.plus(-10, -10), new MusePoint2D(12, 17));
    }
    ...
    public List<ITextComponent> getToolTip(int x, int y) {
        ...
        if (craftAndInstallButton.isVisible() && craftAndInstallButton.hitBox(x, y)) {
            if (craftAndInstallButton.isEnabled()) {
                ret = new TranslationTextComponent("gui.powersuits.craftAndInstall.desc");
            } else {
                ret = new TranslationTextComponent("gui.powersuits.craftAndInstall.disabled.desc");
            }
        }
...
//                            this.recipeBook.func_216761_f()
                    ...
                    MPSPackets.CHANNEL_INSTANCE.sendToServer(new CPlaceRecipePacket(player.openContainer.windowId, this.recipe, Screen.hasShiftDown()));
                }
            }
        }
    }
