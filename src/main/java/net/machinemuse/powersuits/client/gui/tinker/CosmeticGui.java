    ...
    public CosmeticGui(PlayerEntity player) {
        ...
        if (usingCosmeticPresets) {
            ...
            if (mc.isSingleplayer()) {
                allowCosmeticPresetCreation = player.getName().equals(mc.getIntegratedServer().getServerOwner());
            } else {
//                // check if player is top level op
                UserListOpsEntry opEntry = mc.player.getServer().getPlayerList().getOppedPlayers().getEntry(player.getGameProfile());
