...
public class RadialModeSelectionFrame implements IGuiFrame {
            ...
            if (modeChangingCapability.isPresent()) {
                if (modeButtons != null) {
                    ...
                    for (ClickableModule mode : modeButtons) {
                        ItemStack activeModule = stack.getCapability(ModeChangingCapability.MODE_CHANGING).map(m ->m.getActiveModule()).orElse(ItemStack.EMPTY);
    ...
    LazyOptional<IModeChangingItem> getModeChangingCapability() {
        if(!stack.isEmpty())
            return stack.getCapability(ModeChangingCapability.MODE_CHANGING);
        return empty();
    }
    ...
    private void loadItems() {
        if (player != null) {
            ...
            stack.getCapability(ModeChangingCapability.MODE_CHANGING).ifPresent(modeChainging ->
                    {
                        modules.addAll(modeChainging.getInstalledModulesOfType(RightClickCapability.RIGHT_CLICK));

                        for (ItemStack module : modeChainging.getInstalledModulesOfType(MiningEnhancementCapability.MINING_ENHANCEMENT))
    ...
    public void update(double mousex, double mousey) {
        ...
        if (getSelectedModule() != null && selectedModuleOriginal != selectedModuleNew) {
            stack.getCapability(ModeChangingCapability.MODE_CHANGING).ifPresent( modChainging-> {
