...
public class ItemModuleRailgun extends ItemAbstractPowerModule implements IRightClickModule, IPlayerTickModule {
    public ItemModuleRailgun(String regName) {
...
//        addTradeoffPropertyDouble(MPSModuleConstants.VOLTAGE, MPSModuleConstants.RAILGUN_HEAT_EMISSION, 10);
    ...
    public void drawParticleStreamTo(EntityPlayer source, World world, double x, double y, double z) {
        ...
        while (Math.abs(cx - x) > Math.abs(dx / ratio)) {
            world.addParticle(Particles.MYCELIUM, cx, cy, cz, 0.0D, 0.0D, 0.0D);
            cx += dx * 0.1 / ratio;
            cy += dy * 0.1 / ratio;
            cz += dz * 0.1 / ratio;
        }
