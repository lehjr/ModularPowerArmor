 ...
 * 8:26 PM 4/25/13
 */
public class MobRepulsorModule extends AbstractPowerModule implements IPlayerTickModule, IToggleableModule {
    public MobRepulsorModule(EnumModuleTarget moduleTarget) {
        ...
        addBasePropertyDouble(ModuleConstants.MOB_REPULSOR_ENERGY_CONSUMPTION, 2500, "RF");
    }
    ...
    public String getDataName() {
        return ModuleConstants.MODULE_MOB_REPULSOR__DATANAME;
    }

    @Override
    public void onPlayerTickActive(EntityPlayer player, ItemStack item) {
        if (ElectricItemUtils.getPlayerEnergy(player) > ModuleManager.INSTANCE.getOrSetModularPropertyDouble(item, ModuleConstants.MOB_REPULSOR_ENERGY_CONSUMPTION)) {
            if (player.world.getTotalWorldTime() % 20 == 0) {
                ElectricItemUtils.drainPlayerEnergy(player, (int) Math.round(ModuleManager.INSTANCE.getOrSetModularPropertyDouble(item, ModuleConstants.MOB_REPULSOR_ENERGY_CONSUMPTION)));
            }
