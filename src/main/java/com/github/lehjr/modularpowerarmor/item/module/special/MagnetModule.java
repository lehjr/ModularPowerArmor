...
public class MagnetModule extends AbstractPowerModule implements IPlayerTickModule, IToggleableModule {
    public MagnetModule(EnumModuleTarget moduleTarget) {
        ...
        addBasePropertyDouble(ModuleConstants.ENERGY_CONSUMPTION, 0, "RF");
        addTradeoffPropertyDouble(ModuleConstants.POWER, ModuleConstants.ENERGY_CONSUMPTION, 2000);
        addBasePropertyDouble(ModuleConstants.MAGNET_RADIUS, 5);
        addTradeoffPropertyDouble(ModuleConstants.POWER, ModuleConstants.MAGNET_RADIUS, 10);
    }
    ...
    public String getDataName() {
        return ModuleConstants.MODULE_MAGNET__DATANAME;
    }

    @Override
    public void onPlayerTickActive(EntityPlayer player, ItemStack stack) {
        if (ElectricItemUtils.getPlayerEnergy(player) > ModuleManager.INSTANCE.getOrSetModularPropertyDouble(stack, ModuleConstants.ENERGY_CONSUMPTION)) {
            boolean isServerSide = !player.world.isRemote;

            if ((player.world.getTotalWorldTime() % 20) == 0 && isServerSide) {
                ElectricItemUtils.drainPlayerEnergy(player, (int) ModuleManager.INSTANCE.getOrSetModularPropertyDouble(stack, ModuleConstants.ENERGY_CONSUMPTION));
            }
            int range = (int) ModuleManager.INSTANCE.getOrSetModularPropertyDouble(stack, ModuleConstants.MAGNET_RADIUS);
