...
//        .setISTER(() -> PowerFistRenderer::new));
    ...
    public Set<ToolType> getToolTypes(ItemStack itemStack) {
        return itemStack.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY).map(iItemHandler -> {
            ...
            if (iItemHandler instanceof IModeChangingItem) {
                if (!((IModeChangingItem) iItemHandler).getOnlineModuleOrEmpty(MPARegistryNames.PICKAXE_MODULE_REGNAME).isEmpty()) {
                    retSet.add(ToolType.PICKAXE);
                }

                if (!((IModeChangingItem) iItemHandler).getOnlineModuleOrEmpty(MPARegistryNames.AXE_MODULE_REGNAME).isEmpty()) {
                    retSet.add(ToolType.AXE);
                }

                if (!((IModeChangingItem) iItemHandler).getOnlineModuleOrEmpty(MPARegistryNames.SHOVEL_MODULE_REGNAME).isEmpty()) {
    ...
    public boolean hitEntity(ItemStack itemStack, LivingEntity target, LivingEntity attacker) {
        if (attacker instanceof PlayerEntity) {
            itemStack.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY).ifPresent(iItemHandler -> {
                if (iItemHandler instanceof IModeChangingItem) {
                    ((IModeChangingItem) iItemHandler).getOnlineModuleOrEmpty(MPARegistryNames.MELEE_ASSIST_MODULE_REGNAME)
