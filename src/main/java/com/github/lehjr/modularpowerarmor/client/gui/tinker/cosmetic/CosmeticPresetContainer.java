...
//        for (String name :  MPASettings.getModuleConfig().getCosmeticPresets(getItem()).keySet()) {
...
//        }
    ...
    public CosmeticPresetSelectionSubframe createNewFrame(String label, CosmeticPresetSelectionSubframe prev) {
        ...
        return new CosmeticPresetSelectionSubframe(label, new Point2F(newborder.left(), newborder.centery()),  this.itemSelect, newborder);
    }
        ...
        if (enabled) {
            if (button == 0) {
                for (CosmeticPresetSelectionSubframe frame : presetFrames) {
                    if (frame.hitbox((float) x, (float) y))
                        return true;
                }
            }
        }
    ...
    public void render(int mouseX, int mouseY, float partialTicks)  {
        if (isVisible()) {
            ...
            for (CosmeticPresetSelectionSubframe f : presetFrames) {
                f.render(mouseX, mouseY, partialTicks, getzLevel());
            }
            GL11.glPopMatrix();
