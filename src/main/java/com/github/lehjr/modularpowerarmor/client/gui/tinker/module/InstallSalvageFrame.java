...
public class InstallSalvageFrame extends ScrollableFrame implements IRecipeUpdateListener, IRecipePlacer<Ingredient> {
    ...
    public InstallSalvageFrame(
        ...
        float sizex = border.right() - border.left();
        float sizey = border.bottom() - border.top();
...
//                new Colour(0.545D, 0.545D, 0.545D, 1),
        ...
        this.craftAndInstallButton = new ClickableButton(new TranslationTextComponent("gui.modularpowerarmor.craftAndInstall"),
                new Point2F(border.right() - sizex / 2.0F, border.bottom() - sizey / 4.0F),
                true);
        ...
        this.salvageButton = new ClickableButton(new TranslationTextComponent("gui.modularpowerarmor.salvage"),
                new Point2F(border.left() + sizex / 2.0F, border.top() + sizey / 4.0F),
                true);
    ...
    public void init(float left, float top, float right, float bottom) {
        ...
        Point2F arrowsCenter =
                // center of crafting grid
                new Point2F(border.right() -14 - craftingGridSize/2,
                        // center between grid and border bottom
                        (border.finalBottom() - ( border.finalHeight() - craftingGridSize - 7)/2));

        this.forwardArrow.setTargetDimensions(arrowsCenter.plus(10, -10), new Point2F(12, 17));
        this.backArrow.setTargetDimensions(arrowsCenter.plus(-10, -10), new Point2F(12, 17));
    }
    ...
    public void update(double x, double y) {
        ...
        float sizex = border.finalWidth();
        float sizey = border.finalHeight();

        this.craftAndInstallButton.setPosition(
                new Point2F(border.left() + (sizex - craftingGridSize -7)/ 2.0F, border.bottom() - sizey / 2.0F));
        this.installButton.setPosition(
                new Point2F( border.left() + sizex / 2.0F, border.bottom() - sizey / 2.0F));
...
//        this.updateCollections(false);
    ...
    private void drawButtons(int mouseX, int mouseY, float partialTicks) {
        salvageButton.render(mouseX, mouseY, partialTicks, zLevel);
        installButton.render(mouseX, mouseY, partialTicks, zLevel);
        craftAndInstallButton.render(mouseX, mouseY, partialTicks, zLevel);
    }
     ...
     * @param ulOffset
     */
    public void setUlShift(Point2F ulOffset) {
