    ...
    public IntArrayNBT setColourTagMaybe(List<Integer> intList) {
        ...
        return itemSelector.getSelectedItem().getStack().getCapability(ModelSpecNBTCapability.RENDER).map(spec->{
            ...
            if (player.world.isRemote) {
                MPAPackets.CHANNEL_INSTANCE.sendToServer(new MusePacketColourInfo(this.itemSelector.getSelectedItem().getSlotIndex(), this.colours()));
            }
        ...
        if (this.isEnabled()) {
            if (this.selectedSlider != null) {
                ...
                if (colours().length > selectedColour) {
                    ...
                    ClientPlayerEntity player = Minecraft.getInstance().player;
                    if (player.world.isRemote)
                        MPAPackets.CHANNEL_INSTANCE.sendToServer(new MusePacketColourInfo(itemSelector.getSelectedItem().inventorySlot, colours()));
                }
        ...
        boolean removeColour(double x, double y) {
            if (y > this.centery() + 0.5 && y < this.centery() + 8.5 && x > left() + 8 + selectedColour * 8 && x < left() + 16 + selectedColour * 8) {
                ...
                if (intList.size() > 1 && selectedColour <= intList.size() -1) {
                    ...
                    ClientPlayerEntity player = Minecraft.getInstance().player;
                    if (player.world.isRemote)
                        MPAPackets.CHANNEL_INSTANCE.sendToServer(new MusePacketColourInfo(itemSelector.getSelectedItem().getSlotIndex(), IntArrayNBT.getIntArray()));
                }
                return true;
            }
            return false;
