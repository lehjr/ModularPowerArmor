...
import com.github.lehjr.mpalib.client.gui.geometry.FlyFromPointToPoint2F;
...
public class ItemSelectionFrame extends ScrollableFrame {
    ...
    protected List<Point2F> itemPoints;
    ...
    private void loadPoints(int num) {
        float centerx = (border.left() + border.right()) / 2;
        float centery = (border.top() + border.bottom()) / 2;

        itemPoints = new ArrayList();
        List<Point2F> targetPoints = GradientAndArcCalculator.pointsInLine(num,
                new Point2F(centerx, border.top()),
                new Point2F(centerx, border.bottom()), 0, 18);
        for (Point2F point : targetPoints) {
            // Fly from middle over 200 ms
            itemPoints.add(new FlyFromPointToPoint2F(new Point2F(centerx, centery), point, 200));
        }
    ...
    private void loadItems() {
        if (container != null) {
            ...
            Iterator<Point2F> pointiterator = itemPoints.iterator();
    ...
    public boolean mouseClicked(double x, double y, int button) {
        ...
        if (border.containsPoint(x, y)) {
            ...
            int i = 0;
            for (ClickableItem item : itemButtons) {
                if (item.hitBox((float) x, (float) y)) {
